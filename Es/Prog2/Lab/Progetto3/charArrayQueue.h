...
struct charQueue {
    /* capacity == INITIAL CAPACITY*2^n, per qualche numero naturale n >= 0 */
    int capacity;
    /* array, di dimensione capacity, che memorizza gli elementi presenti nella coda */
    char* a; 
    /* numero di elementi presenti nella coda (0 <= size <= capacity) */
    int size;
    /* prima posizione libera in a (dove sarà memorizzato il prossimo elemento della coda (0 <= rear <= capacity-1) */
    int rear; 
    /* posizione in a dove (se size > 0) è memorizzato il primo elemento della coda (0 <= front <= capacity-1) */
    int front; 
