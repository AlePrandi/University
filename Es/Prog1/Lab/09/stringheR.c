...
int conta_caratteriR(const char* pStr, char ch){
	if(*pStr == '\0'){
		...
		return(*pStr == ch) ? 1 + conta_caratteriR(pStr+1, ch) : conta_caratteriR(pStr+1, ch);
	}
}
...
void converti_caratteriR(char* pStr){
	if(*pStr == '\0'){
		...
		return converti_caratteriR(pStr+1);
	}
}


void filtra_carattere(char* pStr, char ch){
	return filtra_carattereR(pStr, pStr, ch);
}
...
void filtra_carattereR(const char* pSrc, char* pDest, char ch){
	if(*pSrc == '\0'){
		*pDest = '\0';
		...
		if(*pSrc == ch){
			return filtra_carattereR(pSrc + 1, pDest, ch);
		}else{
			*pDest = *pSrc;
			return filtra_carattereR(pSrc + 1, pDest + 1, ch);
		}
	}
}