                 ...
                 const size_t i)
{
    ...
    else
    {
        if (i >= aLen)
        {
            return b[i] + max_sum_conR(aLen, a, bLen, b, i + 1);
        }
        else if (i >= bLen)
        {
            return a[i] + max_sum_conR(aLen, a, bLen, b, i + 1);
        }
        else
        {
            return max(a[i], b[i]) + max_sum_conR(aLen, a, bLen, b, i + 1);
        }
    }
}
                 ...
                 const size_t i)
{
    ...
    else
    {
        if (i < aLen && i < bLen)
        {
            return max(a[i - 1], b[i - 1]) + max_sum_covR(aLen, a, bLen, b, i - 1);
        }
        else if (i < aLen)
        {
            return a[i - 1] + max_sum_covR(aLen, a, bLen, b, i - 1);
        }
        else
        {
            return b[i -1] + max_sum_covR(aLen, a, bLen, b, i - 1);
        }
    }
}
                          ...
                          const size_t i)
{
    if(i >= aLen && i >= bLen){
        ...
        if(i >= aLen){
            return(b[i] % 2 == 0) || almeno_uno_pari_conR(aLen, a, bLen, b, i +1); 
        }else if( i >= bLen){
            return(a[i] % 2 == 0) || almeno_uno_pari_conR(aLen, a, bLen, b, i +1);
        }else{
            return(a[i] % 2 == 0 || b[i] % 2 == 0) || almeno_uno_pari_conR(aLen, a, bLen, b, i +1);
        }
    }
}
                          ...
                          const size_t i)
{
    if(i == 0){
        ...
        if(i < aLen && i < bLen){
            return(a[i - 1] % 2 == 0 || b[i - 1] % 2 == 0) || almeno_uno_pari_conR(aLen, a, bLen, b, i - 1); 
        }else if( i < aLen){
            return(a[i - 1] % 2 == 0) || almeno_uno_pari_conR(aLen, a, bLen, b, i - 1);
        }else{
            return(b[i - 1] % 2 == 0) || almeno_uno_pari_conR(aLen, a, bLen, b, i - 1);
        }
    }
}
