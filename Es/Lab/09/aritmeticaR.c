...
int somma_dispariR(int n){
    if(n == 0){
        ...
        return(n % 2 != 0) ? n + somma_dispariR(n - 1) : somma_dispariR(n - 1);
    }
}
...
int prodotto_multipli_in_intervalloR(int n_min, int n, int k){
    ...
    {
        return(n % k == 0) ? (n * prodotto_multipli_in_intervalloR(n_min, n-1, k)) : prodotto_multipli_in_intervalloR(n_min, n - 1, k);
    }
}
